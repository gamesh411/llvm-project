...
void ExceptionAnalyzer::ExceptionInfo::registerException(
  ...
  if (llvm::find(ThrownExceptions, ExceptionType) == ThrownExceptions.end()) {
    ThrownExceptions.push_back(ExceptionType);
  }
}
...
void ExceptionAnalyzer::ExceptionInfo::registerExceptions(
  ...
  if (Exceptions.empty())
    return;
  Behaviour = State::Throwing;
  for (const Type *ExceptionType : Exceptions) {
    if (llvm::find(ThrownExceptions, ExceptionType) == ThrownExceptions.end()) {
      ThrownExceptions.push_back(ExceptionType);
    }
  }
...
ExceptionAnalyzer::ExceptionInfo &ExceptionAnalyzer::ExceptionInfo::merge(
  ...
  for (auto It = Other.ThrownExceptions.begin(),
            End = Other.ThrownExceptions.end();
       It != End; ++It) {
    if (llvm::find(ThrownExceptions, *It) == ThrownExceptions.end()) {
      ThrownExceptions.push_back(*It);
    }
  }
  return *this;
}
...
bool ExceptionAnalyzer::ExceptionInfo::filterByCatch(
  ...
  for (const Type *T : TypesToDelete) {
    auto It = llvm::find(ThrownExceptions, T);
    if (It != ThrownExceptions.end()) {
      ThrownExceptions.erase(It);
    }
  }
...
ExceptionAnalyzer::ExceptionInfo::filterIgnoredExceptions(
  ...
  llvm::SmallSet<const Type *, 8> TypesToDelete;
  ...
  for (auto It = ThrownExceptions.begin(), End = ThrownExceptions.end();
       It != End;) {
    if (TypesToDelete.contains(*It)) {
...
ExceptionAnalyzer::ExceptionInfo ExceptionAnalyzer::throwsException(
  ...
  if (const auto *Throw = dyn_cast<CXXThrowExpr>(St)) {
    ...
    for (unsigned I = 0; I < Try->getNumHandlers(); ++I) {
      ...
      if (!Catch->getExceptionDecl()) {
        ExceptionInfo Rethrown = throwsException(
            Catch->getHandlerBlock(), Uncaught.ThrownExceptions, CallStack);
        ...
        if (Uncaught.filterByCatch(
          ...
          CaughtExceptions.push_back(CaughtType);
    ...
    Results.merge(throwsException(Coro->getExceptionHandler(),
                                  Excs.ThrownExceptions, CallStack));
    for (const Type *Throwable : Excs.ThrownExceptions) {
...
ExceptionAnalyzer::analyzeDispatch(const T *Node) {
  ...
  if (ExceptionList.Behaviour == State::NotThrowing ||
      ExceptionList.Behaviour == State::Unknown)
    return ExceptionList;
